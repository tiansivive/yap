...
| **Status** | **Early / partially broken** |

| File | Lines | Status |
|------|-------|--------|
| `tmp.ts` | 14 | **Stub** — `check` and `infer` both `return 1 as any` |
| `pi.ts` | 92 | **Done** — Lambda chain checking against Pi, implicit insertion, infer+unify fallthrough |
| `check.ts` | 154 | **Partially written, broken** |

**`check.ts` issues:**
- Imports `./struct`, `./match`, `./modal`, `./row` — **none exist on disk** (compile error)
- Lambda checking arm (lines ~42-70) has unfinished code: dangling `tm.annotation` reference, missing closing brackets
- `.otherwise` arm does `import("../inference.v2/tmp")` but returns `1 as any`

**Missing checking.v2 modules** (imported in `check.ts`):
- `struct.ts`
- `match.ts`
- `modal.ts`
- `row.ts`
...
- Fix the broken code in `check.ts` (incomplete match arms, dangling references)
- Implement the 4 missing modules: `struct.ts`, `match.ts`, `modal.ts`, `row.ts`
...
const parseCSTExpr = (src: string) => {
  ...
  if (tree.rootNode.hasError) throw new Error("Parse error");
...
| Decision | Options |
|----------|---------|
| **Mirror v1 tests 1:1?** | *Pro*: diff v1 vs v2 output, catch regressions. *Con*: CST structure differs, some tests won't translate directly |
