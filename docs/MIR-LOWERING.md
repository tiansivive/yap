...
The lowering pass lives in `src/lowering/`, organized as a modular structure:

```
src/lowering/
  lower.ts              — top-level dispatcher, drainAll loop, lowerToMir entry point
  leaf.ts               — Leaf.literal, bound, free, foreign, erase
  struct.ts             — Struct.data, projection, injection
  block.ts              — Block.lower
  functions/
    app.ts              — Functions.App.lower (saturation + indirect calls)
    lambda.ts           — Functions.Lambda.lower (closure conversion)
    closures.ts         — Closure.convert, bundle (shared closure allocation primitive)
    materialize.ts      — materialize, reify, call, partial (closure wrapping for unsaturated foreigns/primops)
    index.ts            — barrel
  continuations/
    reset.ts            — Continuation.Reset.lower
    shift.ts            — Continuation.Shift.lower (worklist-suffix capture, subLower)
    kcall.ts            — Continuation.KCall.lower
    index.ts            — barrel
  matching/
    index.ts            — lower (entry point), compileSubMatrix
    shared.ts           — Pats, Conts, pushVariableLeaf, pushDefaultBranch, ColumnBindings
    variant.ts          — Variant.lower (tag dispatch via Branch)
    literal.ts          — Literal.lower (value dispatch via Branch)
    struct.ts           — Struct.lower (field decomposition + recursive sub-matrix)
  shared/
    primops.ts          — ARITIES (primop arity table)
    helpers.ts          — pushChildrenReversed, notImplemented, extractFields
    freevars.ts         — free variable collection for closure conversion
  monad.ts              — RWSE monad, Frame types, traverse, Do
  context.ts            — LowerCtx, Stamped, bind, bindColumns, ResetCtx, ShiftBodyCtx
  mir.ts                — MIR types (Module, Function, Block, Instr, Terminator, Expr, Allocation)
  patterns.ts           — ts-pattern pattern objects for EB.Term dispatch
  pretty.ts             — display.expr, display.instr, display.module, etc.
```

The pass is a single worklist-driven function (no recursion): global `worklist: Frame[]`, frame types `Lower`, `Cont` (two variants: `"Cont"` for value handlers, `"Cont:sat"` for saturation handlers), and `Delimiter`. Same pattern as `evaluation.v2.ts`. The `drainAll` loop auto-materializes `LowerResult`s (turning pending foreigns/primops into closures) before passing them to `Cont` handlers, unless the handler opts into raw saturation results via `Cont:sat`.

One helper wraps the main drain loop: `subLower(capturedFrames, capturedResults, primer)` in `continuations/shift.ts` for shift's rest-of-reset capture (sub-drives the worklist with a synthetic primer the bottom captured Cont consumes).
...
| `Var(Foreign)`     | ✅     | Accumulates via `LowerResult { tag: "foreign" }`; saturated → direct call, partial → closure wrapper |
| Primitive `App`    | ✅     | Same saturation mechanism as FFI via `LowerResult { tag: "primop" }`                                 |
| `App(Struct, Row)` | ✅     | Record construction → `Alloc` with fields                                                            |
| `App` (general)    | ✅     | Saturated foreign/primop → `Call(direct)` / `PrimOp`; otherwise indirect closure call                |
...
**Closure layout:** `{ __fn: FuncRef, __env: record }`. Uniform calling convention: all closure-called functions take `(env, x)`; caller always passes `[envVar, arg]`. The `bundle` helper in `functions/closures.ts` is the single source of truth for this allocation pattern, used by both lambda closure conversion and partial-application wrapping.

**Call saturation:** Foreigns and primops use a unified worklist-driven accumulator. `Var(Foreign)` pushes a `LowerResult { tag: "foreign" | "primop", args, arity }`. `App` handlers use `Cont:sat` frames to accumulate arguments. When saturated, `materialize` emits `Call(direct)` or `PrimOp`. When unsaturated at a materialization boundary (any non-`Cont:sat` consumer), `materialize` generates a synthetic forwarding closure via `partial`.
...
| EB.Term / Feature | Status | Notes                                                                                                                                                                                                                      |
| ----------------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Match`           | ✅     | Decision-tree compilation (Maranget-style) in `matching/`. Variant, Lit, Struct, Binder, Wildcard. Branch = switch form. Merge via block params (`j` labels). Failure block (`e` label). List pattern not yet implemented. |

### Other terms

| EB.Term / Feature  | Status | Notes                                                                                                                                              |
| ------------------ | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Block`            | ✅     | Statement sequencing via worklist; Let/Expr per statement                                                                                          |
...
- `src/lowering/__tests__/lower.test.ts` — Lit, Var, prim App, struct, proj, inj, Lambda, App (indirect, curried), closure with capture, match (variant/lit/struct/nested/wildcard/default), shift/reset (single/multi-shot, multi-var capture), FFI saturation and partial application; uses `display.module(mod)` for snapshots
...
| **Lowering as pure CFG combinators**               | Today's lowering uses an RWSE monad with pending blocks in State (`M.Pending.open`, `M.Pending.finalize`, `M.Pending.append`). A more "principled" alternative would use a `Fragment` ADT. Worth reconsidering if/when we need fine-grained CFG manipulation (escape analysis, dead-block elimination by hand). For now, the monad-based approach is the same Reader/Writer model — State tracks pending blocks, Writer accumulates completed blocks and functions.                                                                                                 |
...
| **List pattern matching**                          | Match lowering handles Variant, Lit, Struct, Binder, Wildcard. List patterns not yet implemented.                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
> **Status: partially implemented.** Call saturation and partial-application closure wrapping are implemented. Module-level `Declaration` metadata piping and elaboration-time arity computation hookup remain.

### 10.1 Background

Yap has two kinds of foreign names:

- **Primitive operations** (`$add`, `$sub`, etc.) — Hardcoded in `shared/primops.ts` (the `ARITIES` table). Appear as `Var(Foreign("$add"))` in applications. Lowering produces `LowerResult { tag: "primop", name, args, arity }` and accumulates arguments via `Cont:sat` frames. When saturated → `PrimOp` instruction. When unsaturated at a materialization boundary → synthetic closure wrapper.

- **User-declared foreign functions** (`foreign print : String -> Unit`) — Declared in source. The parser produces `{ type: "foreign", variable, annotation }`. Module elaboration (`module.ts:foreign()`) checks the annotation against `Type`, evaluates it to NF, and stores `Var(Foreign(name))` in `ctx.imports`. Lowering produces `LowerResult { tag: "foreign", name, args, arity }` and uses the same saturation mechanism. When saturated → `Call(direct, name, args)`. When unsaturated → closure wrapper.

Both kinds use the **same worklist-driven saturation mechanism** in `functions/materialize.ts`. The `materialize` function dispatches via `reify`: saturated results emit direct calls/primops, unsaturated results generate forwarding closures via `partial`.
...
- **Module-level `Declaration` piping** — `Module` type needs `declarations: Declaration[]`; lowering context needs to receive arity metadata from elaboration.
- **Arity computation hookup** — The `computeArity` algorithm (§10.3) needs to be wired into `module.ts:foreign()` during elaboration. The arity utility (`src/elaboration/arity.ts`) exists but is not yet called.
- **`LowerCtx.arities` population** — Foreign arities must flow from elaboration output into the lowering context so `Leaf.foreign` can produce correct `LowerResult { tag: "foreign", arity }`.
