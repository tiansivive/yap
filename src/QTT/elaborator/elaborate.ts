...
import { displayConstraint, displayContext, displayValue, print } from "./pretty";
import { log } from "./logging";
import { Usages } from "./multiplicity";
...
export type Constraint = { type: "assign"; left: NF.Value; right: NF.Value } | { type: "usage"; computed: Multiplicity; expected: Multiplicity };
...
export function infer(ast: Src.Term): Elaboration<AST> {
	const result = F.pipe(
		...
		chain(ctx => {
			...
			return match(ast)
				.with({ type: "lit" }, ({ value }): Elaboration<AST> => {
					...
					return of<AST>([{ type: "Lit", value }, { type: "Lit", value: atom }, Q.noUsage(ctx.env.length)]);
				})
				...
				.with({ type: "var" }, ({ variable }): Elaboration<AST> => of<AST>(lookup(variable, ctx)))
				.with({ type: "annotation" }, ({ term, ann, multiplicity }) =>
					F.pipe(
						...
						RW.fmap(({ term: [term], type: [type, us] }): AST => [term, type, us]),
					),
				)
				...
				.with({ type: "application" }, ({ fn, arg, icit }) =>
					F.pipe(
						Do,
						bind_("fn", () => chain(infer(fn), icit === "Explicit" ? insertImplicitApps : of)),
							...
							match(fty)
								...
								.otherwise(() => {
									...
									return F.pipe(
										...
										discard(() => tell({ type: "assign", left: fty, right: pi })),
									);
								}),
						),
						...
						chain(({ fn: [ft, fty, fus], arg: [at, aus], pi }) => {
							...
							const val = Eval.apply(ctx.imports, cls, Eval.evaluate(env, ctx.imports, at), q);
				...
				.with({ type: "pi" }, { type: "arrow" }, (pi): Elaboration<AST> => {
					...
					const q = pi.type === "pi" && pi.multiplicity ? pi.multiplicity : Shared.Many;
					...
					return F.pipe(
						...
						RW.fmap(({ ann: [ann, aus], body: [body, [, ...busTail]] }) => [Con.Term.Pi(v, pi.icit, q, ann, body), NF.Type, Q.add(aus, busTail)]),
					);
				})
				.with({ type: "lambda" }, (lam): Elaboration<AST> => {
					const meta: El.Term = El.Var(freshMeta());
					const ann = lam.annotation ? check(lam.annotation, NF.Type) : of<[El.Term, Usages]>([meta, Q.noUsage(ctx.env.length)]);
					return chain(ann, ([tm]) => {
						const va = Eval.evaluate(env, ctx.imports, tm);
						const mva: NF.ModalValue = [va, lam.multiplicity ? lam.multiplicity : Shared.Many];
						...
						return local(
							...
							F.pipe(
								...
								discard(([, , [vu]]) => tell({ type: "usage", expected: mva[1], computed: vu })),
								RW.fmap(([bTerm, bType, [vu, ...us]]): AST => {
									const tm = Con.Term.Lambda(lam.variable, lam.icit, bTerm);
									const pi = Con.Type.Pi(lam.variable, lam.icit, mva, closeVal(ctx, bType));
		...
		discard(([tm, ty, us]) => {
			log("exit", "Result", { Term: print(tm), Type: displayValue(ty), Usages: us });
			return of(null);
		}),
	);
	return result;
...
function check(term: Src.Term, type: NF.Value): Elaboration<[El.Term, Usages]> {
	return F.pipe(
		ask(),
		chain(ctx => {
			log("entry", "Check", { Term: srcPrint(term), Annotation: displayValue(type) });
			return match([term, type])
				.with(
					[{ type: "lambda" }, { type: "Abs", binder: { type: "Pi" } }],
					([tm, ty]) => tm.icit === ty.binder.icit,
					([tm, ty]) => {
						const bType = Eval.apply(ctx.imports, ty.closure, Con.Type.Rigid(ctx.env.length));
						...
						return local(
							...
							F.pipe(
								check(tm.body, bType),
								discard(([, [vu]]) => tell({ type: "usage", expected: ty.binder.annotation[1], computed: vu })),
								RW.fmap(([body, [, ...us]]): [El.Term, Usages] => [Con.Term.Lambda(tm.variable, tm.icit, body), us]),
							),
						);
					},
				)
				.with(
					...
					([tm, ty]) => {
						const bType = Eval.apply(ctx.imports, ty.closure, Con.Type.Rigid(ctx.env.length));
						const ctx_ = bindInsertedImplicit(ctx, ty.binder.variable, ty.binder.annotation);
						return local(
							ctx_,
							F.pipe(
								check(tm, bType),
								discard(([, [vu]]) => tell({ type: "usage", expected: ty.binder.annotation[1], computed: vu })),
								RW.fmap(([tm, [, ...us]]): [El.Term, Usages] => [Con.Term.Lambda(ty.binder.variable, "Implicit", tm), us]),
							),
						);
					},
				)
				.with([{ type: "hole" }, P._], () => of<[El.Term, Usages]>([Con.Term.Var(freshMeta()), []]))
				.otherwise(([tm, _]) =>
					F.pipe(
						infer(tm),
						chain(insertImplicitApps),
						discard(([, inferred]) => tell({ type: "assign", left: inferred, right: type })),
						RW.fmap(([tm, , us]): [El.Term, Usages] => [tm, us]),
					),
				);
		}),
		listen(([[tm, us], csts]) => {
			log("exit", "Result", { Term: print(tm), Usages: us, Constraints: csts.map(displayConstraint) });
			return [tm, us];
		}),
	);
}
...
function insertImplicitApps(node: AST): Elaboration<AST> {
	...
	return F.pipe(
		...
		chain(ctx => {
			...
			return match(node)
				.with([{ type: "Abs", binding: { type: "Lambda", icit: "Implicit" } }, P._, P._], () => of<AST>(node))
...
const lookup = (variable: Src.Variable, ctx: Context): AST => {
	const _lookup = (i: number, variable: Src.Variable, types: Context["types"]): AST => {
		...
		if (types.length === 0) {
			...
			if (free) {
				const [, nf, us] = free;
				return [El.Var({ type: "Free", name: variable.value }), nf, Q.add(us, zeros)];
			}
...
const bind = (context: Context, variable: string, annotation: NF.ModalValue): Context => {
	const [, q] = annotation;
...
const bindInsertedImplicit = (context: Context, variable: string, annotation: NF.ModalValue): Context => {
	const [, q] = annotation;
	const { env, types } = context;
	return {
		...context,
...
const ask = () => RW.liftR<Context, Constraint[], Context>(monoid)(R.ask<Context>());
const chain = RW.getChain<Constraint[]>(monoid);

const Do: Elaboration<{}> = RW.Do(monoid);
const bind_ = RW.bind<Context, Constraint[]>(monoid);

const discard = <A, B>(f: (a: A) => RW.RW<Context, Constraint[], B>) => chain<Context, A, A>(val => RW.fmap(f(val), () => val));

const tell = <R>(constraint: Constraint) => RW.liftW<R, Constraint[], void>(W.tell([constraint]));
...
const local: <A>(f: Context | ((ctx: Context) => Context), rw: RW.RW<Context, Constraint[], A>) => RW.RW<Context, Constraint[], A> = (f, rw) => {
	return (ctx: Context) => {
