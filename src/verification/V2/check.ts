...
export const createCheck = ({ runtime, translation }: CheckDeps) => {
	...
	const check = (tm: EB.Term, ty: NF.Value): V2.Elaboration<VerificationArtefacts> =>
		V2.Do(function* () {
			...
			const viewed = NF.view(ctx, ty);
			const result = match(viewed)
				.with({ kind: "Sealed" }, ({ value: type }) =>
					match([tm, type])
						...
						.with([EB.CtorPatterns.Match, P._], ([term, type]) => matches(term, type))
						.otherwise(([term, type]) => fallback(term, type)),
				)
				.otherwise(({ value: type }) =>
					match(tm)
						.with(EB.CtorPatterns.Mu, term =>
							V2.Do(() => V2.local(c => EB.bind(c, { type: "Mu", variable: term.binding.variable }, type), check(term.body, type))),
						)
						.with(EB.CtorPatterns.Match, term => matches(term, type))
						.otherwise(term => fallback(term, type)),
				);

			function fallback(term: EB.Term, type: NF.Value): V2.Elaboration<VerificationArtefacts> {
				return V2.Do(function* () {
			...
			function matches(term: EB.Term & { type: "Match" }, type: NF.Value): V2.Elaboration<VerificationArtefacts> {
				return V2.Do(function* () {
