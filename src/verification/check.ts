...
const extract = (nf: NF.Value) =>
	match(nf)
		...
		.otherwise(() => ({
			quantity: Q.Many,
			// dummy annotation value, doesn't matter since code has been typechecked already plus it's ignored as liquid neutrals always return true
			liquid: Liquid.Predicate.Neutral(EB.Constructors.Lit({ type: "Atom", value: "Unit" })),
		}));
	...
	V2.Do(function* () {
		...
		const s = match([NF.unwrapNeutral(a), NF.unwrapNeutral(b)])
			...
			.with([{ type: "Modal" }, { type: "Modal" }], ([at, bt]) => {
				...
				const quotedA = NF.quote(ctx, ctx.env.length, at.value);
				const r = EB.Constructors.Lambda("x", "Explicit", implication, quotedA);

				return V2.of(NF.evaluate(ctx, r));
			})
			.with([{ type: "Modal" }, P._], ([at, bt]) =>
				subtype(at, NF.Constructors.Modal(bt, { quantity: Q.Zero, liquid: Liquid.Predicate.Neutral(NF.quote(ctx, ctx.env.length, bt)) })),
			)
			.with([P._, { type: "Modal" }], ([at, bt]) =>
				subtype(NF.Constructors.Modal(at, { quantity: Q.Many, liquid: Liquid.Predicate.Neutral(NF.quote(ctx, ctx.env.length, at)) }), bt),
			)
