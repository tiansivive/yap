...
describe("Lowering: primitives and ops", () => {
	...
	it("lowers Lit(Num(42))", () => {
		...
		expect({ term: EB.Display.Term(term), mir: Pretty.display.module(mod) }).toMatchSnapshot();
	});
...
describe("Lowering: match", () => {
	...
	it("lowers Block (let x=1; let y=2; add(x,y))", () => {
		...
		expect({ term: EB.Display.Term(block), mir: Pretty.display.module(mod) }).toMatchSnapshot();
	});
