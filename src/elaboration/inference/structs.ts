...
export const infer = (struct: Struct): M.Elaboration<EB.AST> =>
	M.tracer.track({ tag: "src", type: "term", term: struct, metadata: { action: "infer", description: "Struct" } }, () => commonStructInference(struct.row));

export const commonStructInference = function* (row: Src.Row): M.Elaboration<EB.AST> {
	const ctx = yield* M.reader.ask();
	const { fields, tail } = yield* EB.Rows.withLabelContext(row, EB.Rows.collect(row));
	...
	const [tm, ty] = yield* match(tail.ty)
		...
		.with(NF.Patterns.Flex, function* (meta) {
			...
			yield* M.constrain({ type: "assign", left: meta, right: schemaTy, lvl: ctx.env.length });
