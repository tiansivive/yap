...
const display = (term: EB.Term, registry: Metas.Registry = {}) => shown(ctx, registry)(() => EB.Display.Term(term));

describe("Displaying elaborated terms", () => {
	describe("Literals", () => {
		it("should display a number", () => {
			const term = EB.Constructors.Lit(Lit.Num(1));
			expect(display(term)).toMatchSnapshot();
		});
	...
	describe("Variables", () => {
		...
		it("should display a meta variable", () => {
			...
			const registry = Metas.register(Metas.empty, { meta: m, annotation: EB.NF.Type });
			expect(display(term, registry)).toMatchSnapshot();
		});
