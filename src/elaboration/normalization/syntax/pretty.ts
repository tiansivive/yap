...
export const display = (value: NF.Value, ctx: EB.DisplayContext, opts = { deBruijn: false }): string => {
	return match(value as NF.Value)
		...
		.with({ type: "Abs" }, ({ binder, closure }) => {
			...
			const printedEnv = extended.env.map(({ nf, name }) => {
				if (nf) {
					...
					return `${name.variable}`; // = ${NF.display(nf, extended, opts)}`; FIXME: this now hangs because of recursive tying of the knot
				}
		...
		.with({ type: "Reset" }, ({ closure }) => {
			return `reset |${displayClosure(closure, ctx, opts)}|`;
		})
		.with({ type: "Shift" }, ({ closure }) => {
			return `shift (${displayClosure(closure, ctx, opts)})`;
		})
