...
const codegen = (env, term) => {
  return match(term).with({ type: "Lit" }, ({ value }) => {
    return match(value).with({ type: "String" }, (s) => `"${s.value}"`).with({ type: "Num" }, (n) => `${n.value}`).with({ type: "Bool" }, (b) => `${b.value}`).with({ type: "unit" }, () => `"unit"`).with({ type: "Atom" }, ({ value: value2 }) => `"${value2}"`).exhaustive();
  }).with({ type: "Var", variable: { type: "Label" } }, ({ variable }) => `${DEFAULT_RECORD_NAME}.${variable.name}`).with({ type: "Var", variable: { type: "Foreign" } }, { type: "Var", variable: { type: "Free" } }, ({ variable }) => {
    ...
    const [, stmts] = block.statements.reduce(
      ([env2, code], stmt) => {
        const code_ = code += " " + Statement.codegen(env2, stmt) + ";";
        const env_ = stmt.type === "Let" ? [stmt.variable, ...env2] : env2;
        return [env_, code_];
      },
      [env, ""]
    );
    ...
    const extract = (row2) => {
      ...
      return [row2.label, ...extract(row2.row)];
    };
    const gen = (r, code) => {
      if (r.type === "empty") {
        return `((() => {${code}
...
const Lit = {
  codegen: (lit) => {
    return match(lit).with({ type: "String" }, (s) => `"${s.value}"`).with({ type: "Num" }, (n) => `${n.value}`).with({ type: "Bool" }, (b) => `${b.value}`).with({ type: "unit" }, () => `"unit"`).with({ type: "Atom" }, ({ value }) => `"${value}"`).exhaustive();
  }
};
const Patterns = {
  codegen: (env, pat, scrutinee) => {
    switch (pat.type) {
      case "Lit":
      ...
      case "List":
        ...
        const [elems, bindings] = pat.patterns.reduce(
          ([conditions, bs5], p, i) => {
            ...
            return [conditions.concat(result[0]), bs5.concat(result[1])];
          },
          [[], []]
        );
        const all = pat.rest ? bindings.concat([[`${scrutinee}.slice(${pat.patterns.length})`, pat.rest]]) : bindings;
...
const Statement = {
  codegen: (env, stmt) => {
    return match(stmt).with({ type: "Expression" }, ({ value }) => codegen(env, value)).with({ type: "Let" }, ({ variable, value }) => {
      const val = codegen([variable, ...env], value);
      return `const ${variable} = ${val}`;
    }).with({ type: "Using" }, (_) => "").otherwise(() => {
